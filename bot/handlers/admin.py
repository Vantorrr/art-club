...
async def test_notifications(message: Message):
    ...
    # Пример 1: Напоминание за 3 дня
    await message.answer(
        "⚠️ <b>Подписка скоро истечет!</b>\n\n"
        "Ваша подписка действует еще <b>3 дня</b>.\n"
        "Не забудьте продлить подписку, чтобы не потерять доступ к клубу.\n\n"
        "Продлить: /start",
        parse_mode="HTML"
    )
    
    # Пример 2: Уведомление об истечении
    await message.answer(
    ...
    await message.answer(
        "ℹ️ <b>Когда отправляются:</b>\n\n"
        "• <b>Напоминание за 3 дня</b> — каждый день в 18:00\n"
        "• <b>Уведомление об истечении</b> — каждый день в 10:00\n\n"
